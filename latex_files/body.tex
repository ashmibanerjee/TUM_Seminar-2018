...
Even though a lot of literature exists for deriving the traveller personality and recommending travel destinations to the travellers based on their pictures that are publicly available from their social media or from the list of destinations that are rated by experts, exploring an individual's publicly available activity on social media to suggest travel destinations to him remains a significantly unexplored problem.
...
Algorithm \ref{algo:1} and algorithm \ref{algo:2} help to summarise the above process.
...
\begin{algorithmic}
  ...
  \EndOutput
  \Procedure{Generating the user profile vectors}{}
  \newline
  \Comment{The clustering of the each of the set of data into the 7 internal categories}
      \For{\texttt{model in models}}
        \State \texttt{clusterVector = cluster(model)}
        \newline
        \Comment{Returns the count of the data points in each cluster category}
         \State \texttt{$userVectors$.append($clustersVector$)}
      \EndFor
      \newline
      \For y in zip(*clusterVectors)
       \State \texttt{$userProfileVectors$.append($sum(y)$) }
      \EndFor
      \newline
       \Return \texttt{$userProfileVector$}
    \EndProcedure
 \end{algorithmic}
\end{algorithm}
...
\begin{algorithmic}
  ...
  \EndOutput
    \Procedure{mapping user profile  coordinates obtained to the seven factor model}{}
          \For{\texttt{model in SevenFactorModel}}
            \State \texttt{score =\\ calculateSimilarityScore(model,userProfileVector)}\\
            \Comment{Returns the similarity score for the model and the cluster}
             \If{$score \geq maximumSimilarScore$}
                    \State $mostSimilarModel\gets model$
                    \State $maximumSimilarScore\gets score$\;
            \EndIf
          \EndFor \\
